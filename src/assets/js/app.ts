...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (event.key === ' ') {
        ...
        if (slot.currentPrizeNumber === slot.grandPrizeStartNo && isReady === false) {
          onWinnerClose();
          onShowReady();
          isReady = true;
          return;
      ...
      if (event.key === 'p') {
        ...
        if (slot.currentPrizeNumber <= slot.grandPrizeStartNo) {
          soundEffects.spin((MAX_REEL_ITEMS - 1) / 10);
          slot.spin(true);
        }
  ...
  // login
  // const urlLogin = 'http://159.89.198.242:2052/login.php?u=mamawards2024&p=mokhtunm61';
  ...
  // const url = 'http://159.89.198.242:2052/guest_all.php?event=mamawards2024';
